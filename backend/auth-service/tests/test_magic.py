...
async def test_magic_verify_creates_user_and_returns_tokens(
    client: AsyncClient,
) -> None:
    with patch("auth_service.routers.magic.send_email", new=_fake_send):
        await client.post(
...
async def test_magic_verify_invalid_token(client: AsyncClient) -> None:
    r = await client.post(
        "/auth/magic/verify", json={"token": "garbage", "intent": "browser"}
    )
...
async def test_magic_verify_marks_used(client: AsyncClient) -> None:
    ...
    # Повторная верификация должна провалиться — подтверждает что used_at установлен.
