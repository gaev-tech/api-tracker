...
1.5 Phasing: реализация делится на два милстоуна.

1.5.1 M5 — Free-тариф и серверный enforcement лимитов; freeze, Pro/Max и платежи отсутствуют. См. `implementation-plan.md` §7.

1.5.2 M6 — Pro и Max, биллинг через ЮКассу, freeze permission-записей, банк Pro-дней, payment-method. См. `implementation-plan.md` §8.

1.5.3 Разделы спеки, относящиеся к платным тирам и freeze, реализуются в M6: §5–§6, §7.1.2–§7.1.7, §8–§14, §15.3–§15.8, §15.15–§15.20, §16.2.3–§16.2.9, §17.2.1–§17.2.6, §18.3, §19.1.2, §19.2.

1.5.4 Разделы спеки для M5: §2.5 (каталог Free), §3 (метрики счёта), §4 (hard reject), §7.1.1 (только колонка `tariff`), §15.1–§15.2 (`tariff show`/`catalog`), §16.2.1–§16.2.2, §17.1.1, §18.2, §19.1.1 (только `tariff` колонка).
...
3.7 Grandfathering: для пользователей с `usage > limit` на момент M5-rollout — существующие permission-записи сохраняются как есть, freeze не применяется (инфраструктура freeze появится в M6). Новые добавления гейтятся §4 на адресате; пользователь с `usage > limit` не принимается в новые сущности до self-revoke. По мере self-revoke `usage` уменьшается; когда `usage ≤ limit` — снова принимается.
