...
**Ответственность.** Долгосрочное хранение всех событий системы и отдача ленты событий через API.
...
**Механика работы:** consumer всех событийных топиков Kafka. При получении события нормализует его и записывает в свою БД. Проверка прав пользователя при чтении ленты — через gRPC к workspace-service (видит ли пользователь задачу/проект/команду, к которой относится событие).
...
**Ответственность.** Единая точка входа публичного API.
...
- PAT-валидация: при каждом запросе вызывается identity-service для проверки токена (с кешированием в Redis на TTL ≈ 60 сек).
...
│   │   ├── identity/
│   │   │   └── gen/                # сгенерированные Go-файлы (pb.go, grpc.pb.go), коммитятся в репо
│   │   ├── workspace/
│   │   │   └── gen/
│   │   ├── automations/
│   │   │   └── gen/
...
- `token` PAT хранится в открытом виде (по продуктовому требованию: токен всегда видим владельцу), но под строгим ограничением доступа к БД identity-service.
...
Оба клиентских приложения вызывают backend через api-gateway (`https://api.example.com`). Аутентификация — Bearer-токен в заголовке `Authorization`.

**Хранение токена в app.** Токен хранится только в памяти Angular (в signal-стейте сервиса), не в localStorage или sessionStorage — снижает риск XSS. При перезагрузке страницы пользователь делает re-login (или использует механизм silent refresh в будущем, если будет введён).

**Docs-клиент.** Не требует аутентификации для большинства страниц (описание продукта, документация API). Для интерактивного «Try it out» в документации — опционально, пользователь может ввести свой PAT, который используется только внутри вкладки (в памяти).
