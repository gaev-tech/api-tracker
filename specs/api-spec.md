...
- `name`: отображаемое имя пользователя. По умолчанию пустая строка.
...
Изменить настройки профиля (имя, тема, язык).
...
Query-параметры:
- `q` — строка поиска (по email-префиксу). Мин. 2 символа.
- `limit` — default 20, max 50.

Фильтрация: текущий пользователь исключается из результатов; возвращаются только verified (`email_verified_at IS NOT NULL`) и active (`is_active = true`) пользователи.
...
Получить массив идентификаторов задач, подходящих под фильтр. Оптимизированный под массовые операции endpoint: возвращает только UUID, без остальных полей задачи. Применяется клиентами, которые выполняют массовые действия над большими выборками — клиент получает список ID постранично, затем вызывает `/tasks/bulk/by-ids` пачками по 1000.

Query-параметры:
- `filter` — RSQL-выражение (опц.).
- `cursor`, `limit` — пагинация, `limit` по умолчанию 1000, максимум 1000.

Видимость та же, что в `GET /tasks` — возвращаются только ID задач, доступных вызывающему.
...
WebSocket-канал для push-уведомлений клиенту в реальном времени.

URL для подключения:
```
wss://api.<domain>/v1/events/stream?token=<PAT>
```

Аутентификация через query-параметр `token` (передача в заголовке `Authorization` невозможна при установке WebSocket из браузера). Для защиты токена от логирования URL в промежуточных компонентах — поддерживается альтернативный вариант: установка соединения с передачей токена в Sec-WebSocket-Protocol заголовке.

При установке соединения сервер:
1. Валидирует PAT через identity-service.
2. Ассоциирует соединение с `user_id`.
3. Сразу пушит первое сообщение с актуальным состоянием.

#### Сообщения от сервера

На старте поддерживается один тип сообщения — актуальный счётчик pending-приглашений пользователя:

```json
{
  "type": "invitations_count",
  "value": 3
}
```
...
На текущий момент клиент не отправляет сообщений; канал однонаправленный (сервер → клиент). В будущем могут добавиться сообщения типа «ping» для keepalive или «subscribe» для настройки подписок.

#### Keepalive и переподключение

- Сервер отправляет `ping` фреймы каждые 30 секунд; клиент должен отвечать `pong`.
- При обрыве соединения клиент переподключается с экспоненциальной задержкой (1с → 2с → 4с → max 30с).
- При успешном переподключении сервер снова пушит актуальное состояние.

#### Масштабирование

WebSocket-соединения хранятся в памяти events-service. Для нескольких реплик сервиса api-gateway использует sticky routing по `user_id`, чтобы соединения одного пользователя попадали в одну реплику (см. `architecture.md`).
