...
| `PUBLIC_REPO_TOKEN` | Personal Access Token с правом `contents:write` в `gaev-tech/apit` (для release.yml; создаётся в M1 при первом релизе). |
...
- Создаёт Release в публичном репо `gaev-tech/apit` через `gh release create` с использованием `PUBLIC_REPO_TOKEN`.
...
1. Создан публичный репо `gaev-tech/apit` (README + Releases).
